<template>
	...
	<div v-if="isOpen" class="fixed inset-0 transition-opacity duration-300 bg-black bg-opacity-50 sidebar_overlay"
		...
		style="z-index: 99999 !important; position: fixed !important; top: 0 !important; left: 0 !important; right: 0 !important; bottom: 0 !important; width: 100vw !important; height: 100vh !important;"
		@click="closeModal">
		...
		<div class="fixed top-0 right-0 h-full flex flex-col transition-transform duration-300 ease-in-out bg-white shadow-2xl content"
			...
			:style="{ ...widthStyle, zIndex: '999999 !important' }"
			@click.stop>
			<!-- Header -->
			<div class="flex items-center justify-between py-3 pl-6 pr-2 modal_header">
				<div v-if="$slots.header" class="flex items-center gap-2" :class="{ 'w-full justify-between': fullHeaderWidth }">
					<slot name="header"></slot>
				</div>
				<h2 v-else class="flex items-center gap-2 modal_title" :class="{ 'w-full justify-between': fullHeaderWidth }">{{ title }} <span v-html="extraHeader"></span></h2>
				<Button type="gray-btn" :onClick="closeModal" classStyle="text-gray-400 hover:text-gray-600 p-1 bg-transparent rounded-full hover:bg-grey-100" style="background: none;">
...
input[type="tel"]:focus:not(.vs__search):not(.dataTableSearch),
