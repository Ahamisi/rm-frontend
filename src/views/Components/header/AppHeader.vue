<template>
  <header
    class="relative z-50 flex items-center justify-between px-4 py-2 bg-white border-b shadow-sm md:hidden desktop">
    <!-- Left: Logo -->
    <div class="flex items-center space-x-2">
      <img src="@/views/Pages/procurement/redesign/app.svg" alt="Logo" class="w-6 h-6" />
      <img src="@/views/Pages/procurement/redesign/logo.svg" alt="Logo" class="w-6 h-6" />
    ...
    <div class="flex items-center space-x-4">
      ...
      <div class="relative cursor-pointer">
        <div @click="openModal">
          <Bell class="w-5 h-5 rotate-45 stroke-2 fill-gray-500 stroke-gray-500" />
          <div
  ...
  <Transition name="slide">
    <div v-if="isMobileMenuOpen" class="fixed inset-0 z-40 flex sm:hidden">
      ...
      <div class="relative w-64 h-full px-4 py-4 bg-white border-l border-gray-200 shadow-lg">
        <LucideX class="absolute w-5 h-5 cursor-pointer top-2 right-2" @click="toggleMobileMenu"></LucideX>
        <!-- Branch Switch -->
        <div class="mb-4">
        ...
        <div class="flex items-center mb-4 space-x-2">
          <div class="flex items-center justify-center w-8 h-8 bg-gray-200 rounded-full">
            <img src="@/views/Pages/procurement/redesign/avatarImage.svg" class="w-6 h-6" />
          </div>
          <div class="flex-1">
  ...
  <header class="relative z-50 hidden block px-6 bg-white border-b md:flex md:justify-between">
    <div class="flex items-center justify-between py-3 mr-auto">
      ...
      <div class="flex items-center">
        ...
        <div class="relative flex items-center gap-2 mr-6">
          <div class="p-1" :class="{ 'bg-gray-200 rounded-full': isAppMenuOpen }" @click="toggleAppMenu">
            <img src="@/views/Pages/procurement/redesign/app.svg" alt="Logo" class="cursor-pointer w-7 h-7" />
          </div>
          ...
          <div v-if="isAppMenuOpen"
            ...
            <ul>
              <li v-for="app in applications" :key="app.name" @click="selectApp(app.name)"
                ...
                <img v-if="selectedApp === app.name" src="@/views/Pages/procurement/redesign/check-circle.svg"
                  alt="Selected" />
              </li>
            </ul>
          </div>

          <img src="@/views/Pages/procurement/redesign/logo.svg" alt="Procurement Logo" class="w-5 h-5" />
          <span class="text-xl font-bold text-blue-700 uppercase department_title_text">{{ selectedApp }}</span>
        </div>
      </div>
      ...
      <div class="relative">
        ...
        <div v-if="isBranchDropdownOpen"
          class="absolute left-0 z-50 py-1 bg-white border-gray-300 rounded-md shadow-xl min-w-32">
          <a v-for="branch in branches" :key="branch.name + branch.id" @click="selectBranch(branch)"
            class="block p-2 capitalize cursor-pointer medium-text text-text-default hover:bg-gray-100">
    ...
    <div class="flex items-center space-x-4">
      ...
      <div class="relative cursor-pointer" @click="toggleProfileMenu">
        <div class="flex items-center justify-center w-8 h-8"
          ...
          <img src="@/views/Pages/procurement/redesign/silhouette.svg" class="w-6 h-6 rounded-full" />
        </div>
        <div v-if="isProfileMenuOpen"
          ...
          <div class="flex items-center px-4 py-2 border-b border-gray-200">
            <div class="relative w-10 h-10">
              <div class="flex items-center justify-center w-10 h-10 bg-gray-200 rounded-full">
                <img src="@/views/Pages/procurement/redesign/avatarImage.svg" class="w-8 h-8 text-gray-500" />
              </div>
          ...
          <div class="mt-2">
            <button class="flex items-center w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
              <img src="@/views/Pages/procurement/redesign/feature-request.svg" class="w-5 h-5 mr-2 text-gray-500" />
              Feature Request
            </button>
            ...
            <button @click="logout" class="flex items-center w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
              <img src="@/views/Pages/procurement/redesign/SignOutIcon.svg" class="w-5 h-5 mr-2 text-gray-500" />
              Sign Out
            </button>
          </div>
        </div>
...
import { useNotificationModal } from "@/views/Composables/procurement/useNotificationModal";
import Notification from "@/views/Components/procurement/notifications/Notification.vue";
import { useAuthStore } from "@/stores/auth";
import { useGlobalStore } from "@/stores/global";
import GlobalSearch from "@/views/Components/procurement/GlobalSearch.vue";
import { useDashboard } from "@/views/Composables/procurement/useDashboard";
import type { Branch } from "@/types";
// import { useNotifications } from "@/views/Components/notifications/";
import { useNotifications } from "@/views/Composables/procurement/useNotifications";
...
const dropdownId = ref("branch-switcher-dropdown");
const isBranchDropdownOpen = computed(() => {
  return store.openDropdownId === dropdownId.value;
});
...
const department_base_routes = [
  ...
  { name: "Customer Success", route: "customer-success" },
  { name: "Sales", route: "sales.dashboard" },
  { name: "Marketing", route: "marketing.dashboard" },
  { name: "Business Development", route: "tech-business-dev.dashboard" },
  { name: "Tech", route: "tech.dashboard" },
  { name: "Accounting", route: "accounting.dashboard" },
...
// const applications = [
//   { name: "Procurement", icon: "@/views/Pages/procurement/redesign/ProcurementIcon.svg" },
//   { name: "Inbound", icon: "@/views/Pages/procurement/redesign/InboundIcon.svg" },
//   { name: "Customer success", icon: "@/views/Pages/procurement/redesign/CustomerSuccessIcon.svg" },
//   { name: "Outbound", icon: "@/views/Pages/procurement/redesign/OutboundIcon.svg" },
//   { name: "Logistics", icon: "@/views/Pages/procurement/redesign/LogisticsIcon.svg" },
// ];
...
const toggleBranchDropdown = () => {
  store.toggleDropdown(dropdownId.value);
};
...
const selectApp = (appName: string) => {
  ...
  const departmentPrefix = `/${appName.toLowerCase()}`;
  ...
  // ✅ Attempt to route if not already in the department
  if (hasRoute(appName)) {
  ...
  const fallbackApp = applications?.find((app: any) =>
  ...
  if (fallbackApp) {
    ...
    if (fallbackRoute) {
      emit("changeDepartment", fallbackApp.name);
