<template>
  <div class="bg-white border border-[#091E4224] rounded-xl shadow-sm overflow-x-auto mx-4">
    <DownloadSuccess :text="'Download Purchase Order Successful'" :visibility="download" />
        ...
        <tr class="text-left border-b border-[#091E4224] text-gray-500 font-thin text-xs">
          <th v-for="header in headers" :key="header.value" class="py-2 px-4 cursor-pointer"
            ...
            <span class="flex items-center space-x-1">
              {{ header.label }}
              <UpDownIcon v-if="header.sortable" :sortKey="sortKey" :isAscending="isAscending"
      ...
      <tbody>
        <tr v-for="(row, index) in sortedData" :key="index"
          ...
          <td v-for="header in headers" :key="header.value" class="py-4 px-4">
            <!-- Assigned Column -->
            <slot :name="header.value" :row="row" v-if="header.value === 'assigned'">
              <p v-if="row.assigned === 'Unassigned'">
                <Tooltip :text="row.assigned">
                  <span class="px-2 py-1 rounded-md medium-text bg-yellow-100 text-yellow-800 line-clamp-2">
                    {{ row.assigned }}
                  </span>
                </Tooltip>
              </p>
              <img v-else class="w-6 h-6 rounded-full" :src="row.assigned.image" :alt="row.assigned" />
            </slot>

            <!-- Action Column -->
            <slot :name="header.value" :row="row" v-else-if="header.value === 'action'">
              <div class="relative">
                <LucideEllipsisVertical class="cursor-pointer text-gray-500 h-5 w-5 hover:text-gray-800"
    ...
    <teleport to="body">
      <div v-if="isDropdownOpen" :style="dropdownStyle"
        ...
        <ul v-if="selectedOrder" class="text-sm text-gray-700 divide-y divide-gray-200">
          <li class="px-4 py-2 hover:bg-gray-100 cursor-pointer flex items-center gap-2 medium-text"
          ...
          <li class="px-4 py-2 hover:bg-gray-100 cursor-pointer flex items-center gap-2 medium-text"
            ...
            <img src="@/views/Pages/procurement/redesign/edit.svg" alt="" class="h-4 w-4" />

            Edit
          </li>
          <li v-if="selectedOrder.assigned == 'Unassigned'"
            ...
            <img src="@/views/Pages/procurement/redesign/assign.svg" alt="" class="h-4 w-4" />
            Assign to Me
          </li>
          <li v-else class="px-4 py-2 hover:bg-gray-100 cursor-pointer flex items-center gap-2 medium-text"
            @click="assignOrder(selectedOrder.id)">
            <!-- <LucideUserPlus class="w-4 h-4" /> -->
            <img src="@/views/Pages/procurement/redesign/assign.svg" alt="" class="h-4 w-4" />
            Unassign
          ...
          <li class="px-4 py-2 hover:bg-gray-100 cursor-pointer flex items-center gap-2 medium-text"
            ...
            <!-- <LucideUserPlus class="w-4 h-4" /> -->
            <img src="@/views/Pages/procurement/redesign/log.svg" alt="" class="h-4 w-4" />
            Activity Log
          </li>
          <!-- v-if="row.assigned == 'Unassigned'" -->
          <li class="px-4 py-2 hover:bg-gray-100 cursor-pointer flex items-center gap-2 medium-text"
            @click="viewOrder(selectedOrder.id)">
            <!-- <LucideUserPlus class="w-4 h-4" /> -->
            <img src="@/views/Pages/procurement/redesign/approve.svg" alt="" class="h-4 w-4" />
            Approve
          </li>
          <li class="px-4 py-2 hover:bg-gray-100 cursor-pointer flex items-center gap-2 medium-text"
            @click="beginDownload">
          ...
          <li class="px-4 py-2 hover:bg-gray-100 cursor-pointer flex items-center gap-2 medium-text"
            ...
            <img src="@/views/Pages/procurement/redesign/trash.svg" alt="" class="h-4 w-4" />
  ...
  <OrderDeleteModal :isOpen="isDeleteOpen" :supplierName="'Supplier X'" @close="
    isDeleteOpen = false;
  selectedOrder = null;
  " @confirm="deleteOrder()" />

  <OrderLog v-model="isOpenLog" @close="isOpenLog = !isOpenLog" :selectedOrder="selectedOrder" />
</template>
...
import DownloadSuccess from "@/views/Components/ui/DownloadSuccess.vue";
import { PurchaseOrder } from "@/types";
import OrderDeleteModal from "@/views/Components/purchase/DeleteModal.vue";
import OrderLog from "@/views/Components/purchase/OrderLog.vue";
import Tooltip from "@/views/Components/ui/Tooltip.vue";
import UpDownIcon from "@/views/Components/icons/UpDownIcon.vue";
