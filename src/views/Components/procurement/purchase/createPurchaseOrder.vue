<template>
  <main class="bg-white px-12 py-3">
    ...
    <div class="text-[#626F86] mb-3 space-x-2">
      <router-link to="/purchase">
        <span class="hover:underline cursor-pointer title-text">Purchase Order</span>
      </router-link>
      <span>/</span>
      <router-link to="/purchase">
      ...
      <span>/</span>
      <router-link to="/">
        <span class="hover:underline cursor-pointer title-text">Create Purchase Order</span>
      </router-link>
    </div>
    ...
    <div>
      <div class="flex items-center justify-between mb-4">
        ...
        <ReusableButton @click="close" bg-color="" text-color="text-[#44546F]" class="medium-text -ml-2">
          <!--  -->
        ...
        <div v-if="selectedOrder" class="flex gap-2 navigation-text">
          ...
          <ReusableButton bgColor="bg-bg-danger" textColor="text-white" @click="handleDelete">
            <LucideTrash class="h-4 w-4" />
            <span>Delete</span>
          </ReusableButton>
          <ReusableButton bgColor="bg-blue-600" textColor="text-white" @click="savePurchaseOrder">
            <span>Update</span>
          </ReusableButton>
      ...
      <div class="p-4 rounded-xl bg-[#F7F8F9]">
        ...
        <div class="grid grid-cols-5 gap-4">
          ...
          <div>
            ...
            <input type="date" v-model="purchaseOrderData.date"
          ...
          <div>
            ...
            <input type="date" v-model="purchaseOrderData.deliveryDate"
              class="text-text-default medium-text w-full p-2 border-[2px] border-[#091E4224] rounded-lg" />
          </div>
          <div>
            <label class="block text-xs text-[#44546F] mb-1 font-medium">Discount (%)</label>
            <input type="number" v-model="purchaseOrderData.discount"
              class="text-text-default medium-text w-full p-2 border-[2px] border-[#091E4224] rounded-lg" />
          </div>
          <div>
            <label class="block text-xs text-[#44546F] mb-1 font-medium">Type</label>
            <select v-model="purchaseOrderData.type"
      ...
      <div class="mt-6 p-4 rounded-xl bg-[#F7F8F9]">
        <div class="flex justify-between items-center mb-4">
          ...
          <button class="border border-[#091E4224] text-[#172B4D] text-xs font-medium px-3 py-1 rounded"
            @click="handleAddItem">
            + Add Item
          </button>
        </div>
          ...
          <div class="overflow-auto rounded-lg border border-[#D0D4D7]">
            <table class="w-full bg-white">
              ...
              <tbody class="text-left text-xs text-[#44546F]">
                <tr v-for="item in purchaseOrderData.items" :key="item.id" class="border-t text-sm text-[#626F86]">
  ...
  <DiscardModal @close="isDiscardOpen = !isDiscardOpen" @confirm="close(false)"
    :action="selectedOrder ? 'Edit Purchase Order' : 'Create Purchase Order'" :isOpen="isDiscardOpen"></DiscardModal>
