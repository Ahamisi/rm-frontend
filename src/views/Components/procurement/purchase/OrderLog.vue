...
<template>
  <Transition name="slide">
    <div v-if="modelValue" class="fixed inset-0 bg-black bg-opacity-50 flex justify-end z-50"
      ...
      <div
        ...
        <div class="p-6 flex justify-between items-center mb-2 border-b">
          <div class="flex gap-2">
            ...
            <img src="@/views/Pages/procurement/redesign/copy.svg" v-if="!copy" alt="copy"
              class="h-6 w-6 cursor-pointer" @click.stop="copy = !copy" />
        ...
        <div class="p-6 relative flex-1 overflow-y-auto pt-6 pb-20">
          ...
          <div v-else v-for="(group, index) in activityGroups" :key="index" class="mb-5">
            ...
            <div class="border-gray-200">
              <div v-for="(activity, idx) in group.activities" :key="idx" class="mb-6 flex items-start md:gap-[10rem]">
                <div class="flex space-x-3 w-3/4">
                  ...
                  <div class="">
                    <p class="table-norm-text w-full text-text-default text-pretty">
                      {{ activity.title }}
                    </p>
                    <p class="w-full text-text-subtle table-sub-text text-nowrap">
                      #{{ activity.orderId }} has been created by
                      <span class="text-[#0055CC] inline">@{{ activity.user }}</span>
                    </p>
                  </div>
                </div>
