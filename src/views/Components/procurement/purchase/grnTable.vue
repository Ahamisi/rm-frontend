<template>
  <div class="bg-white border border-[#091E4224] rounded-xl shadow-sm overflow-x-auto mx-4">
    <DownloadSuccess :text="'Download Purchase Order Successful'" :visibility="downloadPO" />
    ...
    <table class="w-full">
      ...
      <!-- Table Body -->
      <tbody>
        <tr v-for="(row, index) in data" :key="index" class="border-b last:border-0 text-xs font-medium text-[#44546F]">
          <td v-for="header in headers" :key="header.value" class="py-4 px-4">
            ...
            <slot :name="header.value" :row="row" v-if="header.value === 'assigned'">
              ...
              <div v-else class="flex items-center gap-1">
                <img v-if="row.assigned?.image" class="w-6 h-6 rounded-full" :src="row.assigned?.image"
            ...
            <slot :name="header.value" :row="row" v-else-if="header.value === 'action'">
              <div class="relative">
                ...
                <div v-if="openDropdownRowId === row.orderNo"
                  ...
                  <ul class="text-sm text-gray-700 divide-y divide-gray-200">
                    ...
                    <li v-if="row.assigned == 'Unassigned'"
                      class="px-4 py-2 hover:bg-gray-100 cursor-pointer flex items-center gap-2 medium-text"
            ...
            <!-- Status Column -->
            <slot :name="header.value" :row="row" v-else-if="header.value === 'status'">
              <Tooltip :text="row.status">
                <span class="px-2 py-1 rounded-md medium-text line-clamp-2" :class="{
  ...
  <OrderLog v-model="hasSelectedOrder" @close="selectedOrder = null" :selectedOrder="selectedOrder" />
</template>
