...
const tabClass = (type: string) =>
  `pb-2 border-b-2 ${filter.value === type
...
<template>
  <div class="fixed inset-0 bg-black bg-opacity-50 flex justify-end items-center z-50" @click.stop="closeModal">
    <div class="bg-white rounded-lg shadow-lg w-[500px] h-full overflow-y-auto p-4" @click.stop>
      <!-- Header -->
      <div class="border-b pb-3 mb-2 flex items-center justify-between">
        ...
        <button @click="closeModal" class="text-gray-500 hover:text-gray-700 text-2xl cursor-pointer">
          &times;
        </button>
      </div>
      ...
      <div v-if="Object.keys(groupedNotifications).length > 0">
        <div v-for="(group, date) in groupedNotifications" :key="date">
          ...
          <ul>
            <li v-for="notification in group" :key="notification.id" class="flex items-start border-b py-3 relative">
              <!-- Icon -->
              <component :is="getIcon(notification.type)" class="h-5 w-5 mr-3"
              ...
              <div class="flex-1">
                ...
                <span v-if="notification.link" class="text-blue-600 cursor-pointer text-xs">
                  {{ notification.linkText || "View more" }}
                </span>
              </div>

              <!-- Unread dot -->
              <span v-if="!notification.read" class="h-1 w-1 bg-[#E56910] rounded-full absolute right-3 top-5"></span>
            </li>
          </ul>
        </div>
      </div>
