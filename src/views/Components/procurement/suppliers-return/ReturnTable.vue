...
import SuppliersReturnDeleteModal from "@/views/Components/procurement/suppliers-return/DeleteModal.vue";
import SuppliersReturnDropdownMenu from "@/views/Components/procurement/suppliers-return/DropdownMenu.vue";
...
import ReturnLog from "@/views/Components/procurement/suppliers-return/ReturnLog.vue";
...
<template>
  <div class="bg-white border text-lightBlack border-[#091E4224] rounded-xl shadow-sm overflow-x-auto">
    ...
    <table class="w-full min-w-[800px] table-auto">
      <thead>
        <tr class="text-xs font-semibold text-center text-gray-600 bg-gray-100">
          <!-- <th class="p-3 font-semibold">ID</th> -->
          <th class="p-3 cursor-pointer medium-text" @click="$emit('sort', 'id')">
            <div class="flex items-center justify-center">
              <span class="line-clamp-2">Return ID</span>
              <UpDownIcon class="inline w-[12px] ml-1" />
            </div>
          </th>
          <th class="p-3 cursor-pointer medium-text" @click="$emit('sort', 'supplier_name')">
            <div class="flex items-center justify-center">
              <span class="line-clamp-2">Supplier Name</span>
              <UpDownIcon class="inline w-[12px] ml-1" />
            </div>
          </th>
          <th class="p-3 cursor-pointer medium-text" @click="$emit('sort', 'invoice_no')">
            <div class="flex items-center justify-center">
          ...
          <th class="p-3 cursor-pointer medium-text" @click="$emit('sort', 'date')">
            <div class="flex items-center justify-center">
              <span class="line-clamp-2">Date</span>
              <UpDownIcon class="inline w-[12px] ml-1" />
            </div>
          </th>
          <th class="p-3 cursor-pointer medium-text" @click="$emit('sort', 'total')">
            <div class="flex items-center justify-center">
              <span class="line-clamp-2">Total Amount</span>
              <UpDownIcon class="inline w-[12px] ml-1" />
            </div>
          </th>
          <th class="p-3 cursor-pointer medium-text" @click="$emit('sort', 'admin_name')">
            <div class="flex items-center justify-center">
      ...
      <tbody class="medium-text">
        <tr v-for="ret in returns" :key="ret.id" class="relative text-center text-gray-500 border-b hover:bg-gray-50">
          <td class="p-3">
  ...
  <teleport to="body">
    <div v-if="isDropdownOpen" :style="{
      ...
      <ul class="text-sm text-gray-700 divide-y divide-gray-200">
        ...
        <li class="flex items-center gap-1 px-4 py-2 cursor-pointer hover:bg-gray-100 text-nowrap medium-text"
          @click="handleDownload">
          <LucideCloudDownload class="w-4 h-4"></LucideCloudDownload>
