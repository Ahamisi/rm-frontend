<template>
  ...
  <header class="relative z-50 hidden block px-6 bg-white border-b md:flex md:justify-between">
    <div class="flex items-center justify-between px-4 py-3 mr-auto">
    ...
    <div class="flex items-center space-x-4">
      <div class="flex items-center">
        <GlobalSearch v-model="searchTerm" @change="editSearchTerm"></GlobalSearch>
      </div>
      <!-- Notifications -->
...
const selectApp = (appName: string) => {
  ...
  if (fallbackApp) {
    ...
    if (fallbackRoute) {
      emit('changeDepartment', fallbackApp);
