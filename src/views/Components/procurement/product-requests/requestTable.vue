  ...
  <div>
    ...
    <div class="bg-white border border-[#091E4224] rounded-xl shadow-sm overflow-x-auto mx-4">
      <table class="w-full">
        ...
        <!-- Table Body -->
        <tbody>
          <tr v-for="(row, id) in sortedData" :key="id"
            class="border-b last:border-0 text-xs font-medium text-[#44546F]">
            <td v-for="header in headers" :key="header.value" class="px-4 py-4">
              <!-- Assigned Column -->
              <slot :name="header.value" :row="row" v-if="header.value === 'assigned'">
                <p v-if="row.assigned === 'Unassigned'">
                  <span class="px-2 py-1 text-yellow-800 bg-yellow-100 rounded-md medium-text">
                    {{ row.assigned }}
                  </span>
                </p>
    ...
    <RequestView v-model="isViewOpen" @close="closeView" :selectedRequest="selectedRequest" />
    <RequestDeleteModal :isOpen="isDeleteOpen" :requestDesc="selectedRequest?.requestDesc" @close="close"
    ...
    <teleport to="body">
      <div v-if="isDropdownOpen" :style="{
        ...
        <ul v-if="selectedRequest" class="text-sm text-gray-700 divide-y divide-gray-200">
          ...
          <li class="flex items-center gap-2 px-4 py-2 cursor-pointer hover:bg-gray-100 medium-text"
            ...
            <img src="@/views/Pages/procurement/redesign/notify.svg" alt="" class="w-4 h-4" />
...
import RequestView from "@/views/Components/procurement/product-requests/RequestView.vue";
import RequestDeleteModal from "@/views/Components/procurement/product-requests/DeleteModal.vue";
