...
import DeleteModal from "@/views/Components/procurement/payments/DeleteModal.vue";
...
<template>
  <Transition name="slide">
    <div class="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-end" v-if="modelValue" @click.self="close(true)">
      <div class="w-full sm:w-[500px] bg-white h-full flex flex-col shadow-xl overflow-hidden">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          ...
          <LucideX class="w-5 h-5 text-gray-500 cursor-pointer" @click="close(true)" />
        </div>
        ...
        <div class="flex-1 overflow-y-auto px-6 py-4 space-y-6">
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm text-gray-600 mb-1">Purchase Order</label>
              <SelectWithSearchField v-model="form.purchase_order" :options="purchaseOrders" @search="handlePOSearch"
                :is-searching="isSearching" placeholder="Type to search for purchase order" />
            </div>
            <div>
              <label class="block text-sm text-gray-600 mb-1">Supplier</label>
              <UiSelectField v-model="form.supplier" :options="suppliers" placeholder="Select a supplier" />
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <UiSelectField v-model="form.bank_name" :options="banks" placeholder="Select an institution" />
            </div>
            <div>
              <label class="block text-sm text-gray-600 mb-1">Payment Method</label>
              <UiSelectField v-model="form.payment_method" :options="paymentMethods"
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div class="relative">
                ...
                <input v-model="form.total_amount" type="text" placeholder="Enter an amount"
          ...
          <div>
            ...
            <textarea v-model="form.reference" rows="4" placeholder="Add a reference or note for this payment"
              class="w-full border rounded-lg px-3 py-2.5 medium-text focus:ring-2 focus:ring-blue-500 focus:border-transparent"></textarea>
          </div>
        </div>

        <!-- Footer -->
        <div class="border-t px-6 py-4 flex items-center justify-end">
          <button v-if="form.id" @click="isDeleteModalOpen = true"
            class="flex items-center text-white bg-red-500 hover:text-[#44546F] rounded-lg px-2 py-2 text-sm mr-auto">
          ...
          <div class="flex gap-3">
            ...
            <button @click="onSubmit" class="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm">
  ...
  <DiscardModal @close="isDiscardOpen = !isDiscardOpen" @confirm="close(false)"
    :action="form.id ? 'Edit Payment' : 'Create New Payment'" :isOpen="isDiscardOpen" />
  <DeleteModal :isOpen="isDeleteModalOpen" @close="isDeleteModalOpen = false" @confirm="handleConfirmDelete"
