    ...
    <div class="fixed inset-0 bg-black bg-opacity-50 flex justify-end z-50" v-if="modelValue"
      ...
      <div class="bg-white w-full sm:w-1/2 h-full shadow-lg overflow-y-auto transition-transform p-6">
        ...
        <div class="flex justify-between items-center mb-6">
          ...
          <LucideX class="w-6 h-6 cursor-pointer medium text text-text-subtlest" @click="$emit('close')" />
        </div>

        <!-- Fields -->
        <div class="grid grid-cols-2 gap-y-6 gap-x-4 text-sm text-[#172B4D]" v-if="selectedPayment">
