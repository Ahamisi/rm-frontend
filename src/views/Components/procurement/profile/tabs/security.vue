...
import ProfileButton from "@/views/Components/procurement/profile/Button.vue";
import { useUsers } from "@/views/Composables/procurement/useProfile";
...
<template>
  <div class="bg-white rounded-lg shadow-sm space-y-6">
    ...
    <div class="flex justify-between items-center border-b px-6 py-3">
      ...
      <ProfileButton class="bg-profileBlue min-w-[96px] text-white" @click="updatePassword" :disabled="isLoading">
        <span>Update</span>
      </ProfileButton>
    </div>
    ...
    <div class="space-y-6 p-6">
      ...
      <div class="flex items-center gap-8">
        <label for="current-password" class="w-56 text-sm text-gray-600 font-medium">Current Password</label>
        <div class="relative w-full max-w-md">
          <input :type="showCurrent ? 'text' : 'password'" id="current-password" v-model="currentPassword"
            class="w-full border border-gray-300 rounded-md px-4 py-2 pr-10 focus:ring-1 focus:ring-blue-500" />
          <button type="button" @click="toggle('current')"
      ...
      <!-- Row 2 -->
      <div class="flex items-center gap-8">
        <label for="new-password" class="w-56 text-sm text-gray-600 font-medium">New Password</label>
        <div class="relative w-full max-w-md">
          <input :type="showNew ? 'text' : 'password'" id="new-password" v-model="newPassword"
            class="w-full border border-gray-300 rounded-md px-4 py-2 pr-10 focus:ring-1 focus:ring-blue-500" />
      ...
      <!-- Row 3 -->
      <div class="flex items-center gap-8">
        <label for="confirm-password" class="w-56 text-sm text-gray-600 font-medium">Confirm New Password</label>
        <div class="relative w-full max-w-md">
          <input :type="showConfirm ? 'text' : 'password'" id="confirm-password" v-model="confirmPassword"
            class="w-full border border-gray-300 rounded-md px-4 py-2 pr-10 focus:ring-1 focus:ring-blue-500" />
          <button type="button" @click="toggle('confirm')"
            class="absolute top-1/2 right-3 transform -translate-y-1/2 text-gray-400">
  ...
  <SuccessModal v-model="showSuccessModal" title="Password Updated"
    message="Your password has been updated successfully." />
  <ErrorModal v-model="showErrorModal" title="Update Failed" :message="errorMessage" />
</template>
