...
import UiSelectField from "@/views/Components/ui/SelectField.vue";
import UiUpload from "@/views/Components/ui/Upload.vue";
import DiscardModal from "@/views/Components/ui/DiscardModal.vue";
import { useModal } from "@/views/Composables/procurement/useModal";
import { useCategory } from "@/views/Composables/procurement/useCategory";
import { useSupplierForm } from "@/views/Composables/procurement/useSupplierForm";
import { Option } from "@/types";
import { banks as bankMap } from "@/views/helpers/banks";
...
<template>
  <Transition name="slide">
    <div class="fixed inset-0 bg-black bg-opacity-50 flex justify-end z-50" v-if="modelValue"
      @click.self="close(false)">
      <div class="bg-white w-full sm:w-1/2 h-full shadow-lg overflow-y-auto transition-transform p-6">
        <div class="flex justify-between items-center mb-4">
          <h2 class="header-text text-text-default">Create Supplier</h2>
          <LucideX class="w-6 h-6 cursor-pointer text-gray-600" @click="close" />
        </div>

        <UiUpload :imagePlaceholder="`@/views/Pages/procurement/redesign/supplier-placeholder.png`" />

        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="medium-text text-text-subtle mb-1 block">Supplier Name</label>
            <input v-model="supplierForm.name" type="text" placeholder="Enter supplier name"
              class="w-full border-2 border-gray-300 rounded-lg py-1.5 px-3" />
          </div>

          <div>
            <label class="medium-text text-text-subtle mb-1 block">Supplier Type</label>
            <UiSelectField v-model="supplierForm.supplier_type" :options="supplierTypes"
              :placeholder="'Select a type'" />
          </div>

          <div>
            <label class="medium-text text-text-subtle mb-1 block">Category</label>
            <UiSelectField v-model="supplierForm.category" :options="supplierCategories"
              :placeholder="'Select a category'" />
          </div>

          <div>
            <label class="medium-text text-text-subtle mb-1 block">Contact Person</label>
            <input v-model="supplierForm.contact_name" type="text" placeholder="Enter contact person"
              class="w-full border-2 border-gray-300 rounded-lg py-1.5 px-3" />
          </div>
          ...
          <div>
            ...
            <input v-model="supplierForm.address" type="text" placeholder="Enter supplier address"
              class="w-full border-2 border-gray-300 rounded-lg py-1.5 px-3" />
          </div>

          <div>
            <label class="medium-text text-text-subtle mb-1 block">Email</label>
            <input v-model="supplierForm.email" type="email" placeholder="Enter email"
              class="w-full border-2 border-gray-300 rounded-lg py-1.5 px-3" />
          </div>

          <div>
            <label class="medium-text text-text-subtle mb-1 block">Phone</label>
            <input v-model="supplierForm.phone" type="tel" placeholder="e.g. 0801 234 5678"
              class="w-full border-2 border-gray-300 rounded-lg py-1.5 px-3" />
          </div>

          <div>
            <label class="medium-text text-text-subtle mb-1 block">Opening Balance</label>
            <input v-model="supplierForm.opening_balance" type="number" placeholder="₦ Enter balance"
              class="w-full border-2 border-gray-300 rounded-lg py-1.5 px-3" />
          </div>

          <div>
            <label class="medium-text text-text-subtle mb-1 block">Balance Date</label>
            <input v-model="supplierForm.balance_date" type="date"
        ...
        <div class="grid grid-cols-2 gap-4 mt-2">
          <div>
            <label class="medium-text text-text-subtle mb-1 block">Bank Name</label>
            <UiSelectField v-model="supplierForm.bank_name" :options="supplierBanks" :placeholder="'Select bank'" />
          </div>
          <div>
            <label class="medium-text text-text-subtle mb-1 block">Bank Code</label>
            <input v-model="supplierForm.bank_code" type="text" placeholder="--"
              class="w-full border-2 border-gray-300 rounded-lg py-1.5 px-3" />
          </div>
          <div>
            <label class="medium-text text-text-subtle mb-1 block">Account Number</label>
            <input v-model="supplierForm.account_number" type="text" placeholder="Enter account number"
              class="w-full border-2 border-gray-300 rounded-lg py-1.5 px-3" />
          </div>
          <div>
            <label class="medium-text text-text-subtle mb-1 block">Account Holder</label>
            <input v-model="supplierForm.account_holder" type="text" placeholder="--"
              class="w-full border-2 border-gray-300 rounded-lg py-1.5 px-3" />
          </div>
        </div>
      </div>
      <div class="border-t sm:w-1/2 my-4 flex justify-between items-center">
        <button @click="$emit('delete:supplier')" class="bg-bg-danger px-6 py-2 medium-text rounded-lg"
          :disabled="isSubmitting">
          <LucideTrash2 class="text-white h-4 w-4"></LucideTrash2>
        </button>
        <div class="flex justify-end mt-6 gap-4">
          <button @click="close(true)" class="bg-gray-200 px-6 py-2 medium-text rounded-lg" :disabled="isSubmitting">
            Cancel
          </button>
          <button @click="onSubmit" class="bg-blue-600 text-white medium-text px-6 py-2 rounded-lg"
            :disabled="isSubmitting">
            Create
          </button>
        </div>
  ...
  <DiscardModal @close="isDiscardOpen = !isDiscardOpen" @confirm="close" :action="'Create New Supplier'"
    :isOpen="isDiscardOpen"></DiscardModal>
</template>
