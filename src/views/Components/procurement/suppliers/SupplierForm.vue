...
import UiUpload from "@/views/Components/procurement/ui/Upload.vue";
...
import TrashIcon from "@/views/Components/procurement/icons/TrashIcon.vue";
import DeleteModal from "@/views/Components/procurement/suppliers/DeleteModal.vue";
...
<template>
  <Transition name="slide">
    <div class="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-end" v-if="modelValue"
      @click.self="close(false)">
      <div class="w-full sm:w-[500px] bg-white h-full flex flex-col shadow-xl overflow-hidden">
        <!-- Header -->
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <h2 class="header-text text-text-default">
            {{ supplierForm.id ? "Edit Supplier" : "Create New Supplier" }}
          </h2>
          <LucideX class="w-5 h-5 text-gray-500 cursor-pointer" @click="close()" />
        </div>
        ...
        <div class="flex-1 overflow-y-auto px-6 py-4 space-y-6">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label class="block text-sm text-gray-600 mb-1">Balance Date</label>
          ...
          <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <UiSelectField :displayIcon="LucideBuilding2" v-model="supplierForm.bank_name" :options="supplierBanks"
                  :placeholder="'Select bank'" />
              </div>

              <div>
                <label class="block text-sm text-gray-600 mb-1">Bank Code</label>
        ...
        <div class="border-t px-6 py-4 flex items-center justify-end">
          ...
          <div class="flex gap-3">
            <button @click="close(true)" class="bg-gray-100 text-gray-700 px-4 py-2 rounded-lg text-sm"
              :disabled="isSubmitting">
              Cancel
            </button>
            <button @click="onSubmit" class="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm disabled:opacity-50"
              :disabled="isSubmitting">
  ...
  <DiscardModal @close="isDiscardOpen = !isDiscardOpen" @confirm="close(false)" :action="'Create New Supplier'"
    :isOpen="isDiscardOpen"></DiscardModal>

  <DeleteModal :isOpen="isDeleteModalOpen" :supplierName="supplierForm.name" @close="isDeleteModalOpen = false"
