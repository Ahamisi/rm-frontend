...
// import SupplierDeleteModal from "@/views/Components/suppliers/SupplierDeleteModal.vue";
...
import { useStatus } from "@/views/Composables/procurement/useStatus";
import EditIcon from "@/views/Components/procurement/icons/EditIcon.vue";
import PaymentIcon from "@/views/Components/procurement/icons/PaymentIcon.vue";
...
<template>
    ...
    <table class="w-full min-w-[800px] table-auto">
      <thead>
        <tr class="bg-gray-100 text-center medium-text text-text-default">
          ...
          <th class="p-3 medium-text cursor-pointer" @click="$emit('sort', 'name')">
            Supplier Name
            <UpDownIcon class="inline w-[12px] ml-1" />
          </th>
          <th class="p-3 medium-text cursor-pointer" @click="$emit('sort', 'contact_name')">
            Contact Name
            <UpDownIcon class="inline w-[12px] ml-1" />
          </th>
          <th class="p-3 medium-text">Phone</th>
          <th class="p-3 medium-text cursor-pointer" @click="$emit('sort', 'category')">
            Category
            <UpDownIcon class="inline w-[12px] ml-1" />
          </th>
          <th class="p-3 medium-text cursor-pointer" @click="$emit('sort', 'status')">
            Status
            <UpDownIcon class="inline w-[12px] ml-1" />
          </th>
  ...
  /> -->
  <SupplierDeleteModal :isOpen="isModalOpen" :supplierName="selectedSupplier?.supplierName" @close="isModalOpen = false"
  ...
  <teleport to="body">
    <div v-if="isDropdownOpen" :style="{
      ...
      <ul class="text-sm text-gray-700 divide-y divide-gray-200">
        ...
        <li @click="handleEdit" class="px-4 py-2 hover:bg-gray-100 cursor-pointer flex items-center gap-2 medium-text">
          <EditIcon class="h-4 w-4" />
          Edit
        </li>

        <li @click="handleAddPurchaseOrder"
          class="px-4 py-2 hover:bg-gray-100 cursor-pointer flex items-center gap-2 medium-text">
          <!-- <img src="@/views/Pages/procurement/redesign/plus-circle.svg" alt="" class="h-4 w-4" /> -->
          <LucidePlus class="w-4 h-4" />
          Add Purchase Order
        </li>
        <li @click="handleAddPayment"
          class="px-4 py-2 hover:bg-gray-100 cursor-pointer flex items-center gap-2 medium-text">
          <!-- <img src="@/views/Pages/procurement/redesign/plus-circle.svg" alt="" class="h-4 w-4" /> -->
          <!-- <LucidePlus class="w-4 h-4" /> -->
