...
import { useSearch } from "@/views/Composables/procurement/useSearch";
import { useSuppliers } from "@/views/Composables/procurement/useSuppliers";
import { useFilter } from "@/views/Composables/procurement/useFilter";
...
<template>
  <div class="flex border-2 border-gray-300 rounded-lg overflow-hidden shadow-sm w-[281px] h-[34px]">
    ...
    <div class="flex items-center gap-2 px-4 py-2 bg-gray-50 w-full">
      ...
      <InputBox :model-value="props.modelValue" @update:modelValue="$emit('update:modelValue', $event)"
        placeholder="Search" custom-styles="bg-transparent outline-none w-full text-sm text-gray-700" />
    </div>
    ...
    <SearchDropdown :isOpen="modalTypeOpen == 'filter'" :toggle="() => toggleModal(`filter`)" :alignLeft="true">
      <template #trigger>
        <div class="border-l-2 border-gray-300 px-4 py-2 flex items-center bg-gray-50">
          <LucideListFilter class="w-4 h-4 text-gray-600" @click="emit('filterModalOpen')" />
        </div>
      </template>
    ...
    <SearchDropdown :isOpen="modalTypeOpen == 'sort'" :toggle="() => toggleModal(`sort`)" :alignLeft="false">
      <template #trigger>
        <div class="border-l-2 border-gray-300 px-4 py-2 flex items-center bg-gray-50">
          <LucideArrowUpDown class="w-4 h-4 text-gray-600" @click="emit('filterOpen')" />
        </div>
      </template>
      <!-- <div class="p-4">
