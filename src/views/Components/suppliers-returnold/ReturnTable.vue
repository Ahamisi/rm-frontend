...
import SuppliersReturnDeleteModal from "@/views/Components/suppliers-return/DeleteModal.vue";
import SuppliersReturnDropdownMenu from "@/views/Components/suppliers-return/DropdownMenu.vue";
...
import ReturnLog from "@/views/Components/suppliers-return/ReturnLog.vue";
...
<template>
  <div class="bg-white border text-lightBlack border-[#091E4224] rounded-xl shadow-sm overflow-x-auto">
    <DownloadSuccess :text="`Download Supplier's Return Successful`" :visibility="download" />
    <table class="w-full min-w-[800px] table-auto">
      <thead>
        <tr class="bg-gray-100 text-center text-xs font-semibold text-gray-600">
          <!-- <th class="p-3 font-semibold">ID</th> -->
          <th class="p-3 medium-text cursor-pointer" @click="$emit('sort', 'id')">
            <div class="flex items-center justify-center">
              <span class="line-clamp-2">Return ID</span>
              <UpDownIcon class="inline w-[12px] ml-1" />
            </div>
          </th>
          <th class="p-3 medium-text cursor-pointer" @click="$emit('sort', 'supplier_name')">
            <div class="flex items-center justify-center">
              <span class="line-clamp-2">Supplier Name</span>
              <UpDownIcon class="inline w-[12px] ml-1" />
            </div>
          </th>
          <th class="p-3 medium-text cursor-pointer" @click="$emit('sort', 'invoice_no')">
            <div class="flex items-center justify-center">
          ...
          <th class="p-3 medium-text cursor-pointer" @click="$emit('sort', 'date')">
            <div class="flex items-center justify-center">
              <span class="line-clamp-2">Date</span>
              <UpDownIcon class="inline w-[12px] ml-1" />
            </div>
          </th>
          <th class="p-3 medium-text cursor-pointer" @click="$emit('sort', 'total')">
            <div class="flex items-center justify-center">
              <span class="line-clamp-2">Total Amount</span>
              <UpDownIcon class="inline w-[12px] ml-1" />
            </div>
          </th>
          <th class="p-3 medium-text cursor-pointer" @click="$emit('sort', 'admin_name')">
            <div class="flex items-center justify-center">
      ...
      <tbody class="medium-text">
        <tr v-for="ret in returns" :key="ret.id" class="border-b text-gray-500 hover:bg-gray-50 text-center relative">
          <td class="p-3">
  ...
  <teleport to="body">
    <div v-if="isDropdownOpen" :style="{
      ...
      <ul class="text-sm text-gray-700 divide-y divide-gray-200">
        ...
        <li @click="handleReturnLog"
          class="px-4 py-2 hover:bg-gray-100 cursor-pointer flex items-center gap-2 medium-text">
          <img src="@/views/Pages/procurement/redesign/log.svg" alt="" class="h-4 w-4" />
          Activity Log
        </li>
        <li class="px-4 py-2 hover:bg-gray-100 text-nowrap cursor-pointer flex items-center gap-1 medium-text"
  ...
  <ReturnLog v-model="isLogOpen" @close="isLogOpen = false" :selectedReturn="selectedReturn" />
</template>
