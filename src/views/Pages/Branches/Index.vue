<template>
    ...
    <div class="erp_dashboard_wrapper">
        <!-- Header -->
        <PageTitle title="Location / Branches" class="px-6" />

        <div class="min-h-[calc(100vh-190px)] px-6 bg-white">
            <Datatable :url="url" :filterByDate="false" :columns="newColumns" pageName="Branch" :key="childKey">
                <template v-slot:header_actions>
                    <button @click.prevent="isCreateBranchModalOpen = true"
                ...
                <template #column="col">
                    <span v-if="col.props.column.field == 'actions'">
                        <div class="flex items-center justify-center gap-3">
                            <button type="button" @click.prevent="editBranch(col.props.row)">
                            ...
                            <button type="button" @click.prevent="openConfirmDeleteBranch(col.props.row)">
                                <svg width="25" height="24" viewBox="0 0 25 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" clip-rule="evenodd"
        ...
        <!-- discard delete branch modal -->
        <UniversalCenteredModal :show="showDiscardDeleteBranchModal" @close="showDiscardDeleteBranchModal = false">
            <template #header>
            ...
            <template #body>
                <p class="discard_merge_alert_body_text">
                    You are about to delete this branch <strong>“{{ deleteBranchFormData.name }}”</strong>. This
            ...
            <template #footer>
                <div class="flex justify-end space-x-3 discard_merge_alert_footer">
                    <button @click="showDiscardDeleteBranchModal = false" type="button" class="cancel_btn">
                        Cancel
                    </button>
                    <button @click="deleteBranch" type="button" class="text-white delete_btn">
                        Delete Branch
                    </button>
                </div>
            </template>
        ...
        <!-- deleted branch success message modal -->
        <UniversalCenteredModal :show="showDeletedBranchSuccessModal" @close="closeDeleteBranchSuccessModal">
            <template #body>
                ...
                <h3 class="text-center">Branch Deleted</h3>
                <p class="text-center discard_merge_alert_body_text">
                    The branch <strong>“{{ deleteBranchFormData.name }}”</strong> has been successfully deleted from the
                    system.
                </p>
            </template>
            ...
            <template #footer>
                <div class="flex justify-end space-x-3 discard_merge_alert_footer">
                    ...
                    <button @click="closeDeleteBranchSuccessModal" type="button" class="text-white approve_btn">
                        Done
                    </button>
                </div>
            </template>
        ...
        <Create :isOpen="isCreateBranchModalOpen" @created="childKey++; isCreateBranchModalOpen = false"
            @close="isCreateBranchModalOpen = false" @discarded="isCreateBranchModalOpen = false" />
        <Edit :isOpen="isEditBranchModalOpen" :branch="editBranchFormData"
            @updated="childKey++; isEditBranchModalOpen = false" @close="isEditBranchModalOpen = false"
            @discarded="isEditBranchModalOpen = false" />
    </div>
</template>
...
const showDiscardDeleteBranchModal = ref(false);
const showDeletedBranchSuccessModal = ref(false);
...
const url = computed(() => import.meta.env.VITE_API_URL + 'branches');
...
const deleteBranchFormData = ref({
    id: 0,
    name: '',
    address: ''
})
...
const deleteBranch = (id) => {
    showDiscardDeleteBranchModal.value = false;
    if (!deleteBranchFormData.value?.id) {
        toast.error("Please select a branch to delete");
        return;
    }
    loading.value = true;
    axios.delete(import.meta.env.VITE_API_URL + `branches/${deleteBranchFormData.value.id}`).then(response => {
        showDeletedBranchSuccessModal.value = true;
    }).catch(err => {
        let errorMessage = '';
...
const closeDeleteBranchSuccessModal = () => {
    deleteBranchFormData.value = {
        id: 0,
        name: '',
        address: ''
    }
    showDeletedBranchSuccessModal.value = false;
    loading.value = false;
    childKey.value++;
}

const openConfirmDeleteBranch = (branch) => {
    deleteBranchFormData.value = branch;
    showDiscardDeleteBranchModal.value = true;
}


const editBranch = (branch) => {
    editBranchFormData.value = branch;
