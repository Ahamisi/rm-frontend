<template>
                ...
                <div class="w-full h-screen overflow-hidden bg-white shadow-xl">
                    ...
                    <div class="flex items-center justify-between p-4 border-b border-gray-200">
                        <div class="flex items-center flex-1 space-x-4">
                            <h2 class="mr-auto modal_title">Create Product</h2>
                    ...
                    <div class="p-6 overflow-y-auto max-h-[85vh] h-[85vh]">
                        ...
                        <div v-if="currentStep === 1" class="space-y-6">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                                    <div>
                                        <label class="block mb-2 text-sm font-medium text-gray-700">Discount on
                                            Carton
                                            (%)</label>
    ...
    <UniversalCenteredModal :show="showDiscardCreateProductModal" @close="showDiscardCreateProductModal = false">
        <template #header>
        ...
        <template #body>
            <p class="discard_merge_alert_body_text">
                You are about to leave the <strong>"Create New Product"</strong> process. Any unsaved
                information
                will
                be lost.
            </p>
        ...
        <template #footer>
            <div class="flex justify-end space-x-3 discard_merge_alert_footer">
                <button @click="showDiscardCreateProductModal = false" type="button" class="cancel_btn">
                    Cancel
                </button>
                <button @click="handleDiscardCreateProduct" type="button" class="discard_btn">
                    Discard Changes?
                </button>
            </div>
        </template>
    ...
    <UniversalCenteredModal :show="showCreateProductSuccessModal" @close="showCreateProductSuccessModal = false">
        <template #body>
            ...
            <h4 class="text-center">Product Created Successfully</h4>
            <p class="discard_merge_alert_body_text">
                The product <strong>“{{ formData.name }}”</strong> has been successfully added to the system.
            </p>
        </template>

        <template #footer>
            <div class="flex justify-end space-x-3 discard_merge_alert_footer">
                <button @click="handleDiscardCreateProduct" type="button" class="approve_btn">
                    Done
                </button>
            </div>
        </template>
...
const showDiscardCreateProductModal = ref(false);
...
const handleShowDiscardCreateProductModal = () => {
    if (hasChanged.value) {
        showDiscardCreateProductModal.value = true;
    } else {
        handleDiscardCreateProduct();
    }
}

const handleDiscardCreateProduct = async () => {
    formData.value = {
        name: "",
        description: "",
        category_id: "",
    ...
    reset();
    closeModal();
    showDiscardCreateProductModal.value = false;
    emit('close')
}
...
const submitForm = (): void => {
    ...
    axios({ url: import.meta.env.VITE_API_URL + 'products', data: formData.value, method: 'POST' })
        .then(resp => {
            // formData.value = {
            //     name: "",
            //     description: "",
