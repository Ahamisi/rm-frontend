<template>
    ...
    <div>
        <!-- Transfer Product To Branch Modal -->
        <SideBarModal title="Transfer Information to other Branch" :extraHeader="getTitle" ref="transferToBranchModal"
            :is-open="isTransferProductModalOpen" width="xl" :initial-form-data="transfer"
            :show-discard-warning="showDiscardTransferProductWarningModal"
            ...
            <template #default="{ formData, updateFormData }">
                <div class="space-y-6">
                    ...
                    <div class="mb-6">
                        ...
                        <div class="relative">
                            <select v-model="formData.to_branch"
                                class="w-full max-w-md px-4 py-2 text-gray-700 bg-white border border-gray-300 rounded-md"
                                @input="updateFormData({
                                    to_branch: parseFloat($event.target.value) || 0,
                                })">
        ...
        <UniversalCenteredModal :show="showDiscardTransferProductWarningModal"
            ...
            <template #body>
                <p class="discard_merge_alert_body_text">
                    You are about to leave the <strong>"Product Transfer To Other Branch"</strong> process. Any unsaved
                    information
                    will
                    be lost.
                </p>
...
const transferToBranchModal = ref(null);
...
const handleDiscardProductTransfer = () => {
    ...
    if (isTransferProductModalOpen.value) transferToBranchModal.value?.closeModal();
    loading.value = false;
    transfer.value = {
        to_branch: '',
        batches: []
...
const createTransfer = () => {
    loading.value = true;
    axios.post(import.meta.env.VITE_API_URL + `products/${product.value.id}/transfer`, transfer.value)
        .then(res => {
