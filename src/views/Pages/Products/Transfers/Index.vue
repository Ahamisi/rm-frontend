<template>
    ...
    <div class="erp_dashboard_wrapper">
        ...
        <!-- Header -->
        <PageTitle title="Pending Items / Pending Product Transfer" class="px-6" />

        <div class="px-6 min-h-[calc(100vh-190px)]">
            <Datatable :url="url" :filterByDate="true" :columns="newColumns" pageName="PendingProducts">
                <template v-slot:header_actions>
                    ...
                    <button @click="download"
                        ...
                        <span>Download Pending Transfer Report</span>
                    </button>
                    <!-- </div> -->
                </template>
...
const url = computed(() => import.meta.env.VITE_API_URL + 'admin/pending_products');

const download = () => {
    loading.value = true;
    let url = `${import.meta.env.VITE_API_URL}admin/pending_products?branch_id=${authStore.selectedBranch.id}&download=1&current_branch=${authStore.selectedBranch.id}`
    axios.get(url)
        .then(res => {

            let filename = 'pending_product_transfer';
            filename += `.xlsx`;
        ...
        .catch(err => {
            new Blob([err.response.data]).text().then(res => {
                let errorMessage = JSON.parse(res).error;
