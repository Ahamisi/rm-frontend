<template>
  ...
  <div class="px-6 erp_dashboard_wrapper">
      <!-- Header -->
    <PageTitle title="Orders & Fulfilment / Customer Support Issues" />
    ...
    <Datatable
      ...
      :key="childKey"
      pageName="CustomerSuccessSupportIssues"
    >
        <template #column="col">
          <!-- Order No -->
...
// Computed URL for customer support issues
const delivered_url = computed(() => {
  const params = new URLSearchParams({
    filter: JSON.stringify({ state: 'Support Issue', department: 'customer-success' })
  });
  return `/customer-success/support-issues?${params.toString()}`;
});
