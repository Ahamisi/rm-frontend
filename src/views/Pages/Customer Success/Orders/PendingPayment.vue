<template>
  ...
  <div class="px-6 erp_dashboard_wrapper">
      <!-- Header -->
    <PageTitle title="Orders & Fulfilment / Orders Pending Payment" />
    ...
    <div class="flex items-center justify-center min-h-[calc(100vh-190px)]" v-if="!hasSearched">
      <div class="intro_search_wrapper">
        ...
        <p class="my-4 text-center">Please search by Order Ref No, Customer Name or Store Name to load orders pending payment</p>
    ...
    <div class="min-h-[calc(100vh-190px)]" v-else>
      <Datatable
        ...
        :key="childKey"
        pageName="CustomerSuccessPendingPayment" 
          ...
          <span v-else-if="col.props.column.field === 'actions'">
            <TableActionDropdown :rowData="col.props.row" :key="col.props.row.id" :noDividers="true">
              <template #default="{ selectedItem, closeDropdown }">
                ...
                <li @click="confirmOrderPayment(selectedItem); closeDropdown()" 
                    class="flex items-center gap-2 px-4 py-2 cursor-pointer hover:bg-gray-100 medium-text">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM10 17L5 12L6.41 10.59L10 14.17L17.59 6.58L19 8L10 17Z" fill="#626F86"/>
                  </svg>
                  Confirm Order Payment
                </li>
                
                <!-- Cancel Order -->
                <li @click="cancelOrder(selectedItem); closeDropdown()" 
                    class="flex items-center gap-2 px-4 py-2 cursor-pointer hover:bg-gray-100 medium-text">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM13 17H11V15H13V17ZM13 13H11V7H13V13Z" fill="#626F86"/>
                  </svg>
                  Cancel Order
                </li>
...
// Computed URL for orders pending payment
const delivered_url = computed(() => {
  const params = new URLSearchParams({
    search_by: selectedSearchBy.value || 'order',
    search: search.value.trim(),
    filter: JSON.stringify({ state: 'Pending Payment', department: 'customer-success' })
  });
  return `/customer-success/pending-payment?${params.toString()}`;
});
...
const confirmOrderPayment = (order: any) => {
  console.log('Confirm order payment:', order);
  // Implement confirm order payment logic
};
