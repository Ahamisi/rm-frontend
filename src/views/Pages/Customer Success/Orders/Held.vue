<template>
  ...
  <div class="px-6 erp_dashboard_wrapper">
      <!-- Header -->
    <PageTitle title="Orders & Fulfilment / Held Orders" />
    ...
    <Datatable
      ...
      :key="childKey"
      pageName="CustomerSuccessHeldOrders"
    >
        <template #column="col">
          <!-- Order No -->
          ...
          <span v-else-if="col.props.column.field === 'actions'">
            <TableActionDropdown :rowData="col.props.row" :key="col.props.row.id" :noDividers="true">
              <template #default="{ selectedItem, closeDropdown }">
                ...
                <!-- Assign to me -->
                <li @click="assignToMe(selectedItem); closeDropdown()" 
                    class="flex items-center gap-2 px-4 py-2 cursor-pointer hover:bg-gray-100 medium-text">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
// Computed URL for held orders
const delivered_url = computed(() => {
  const params = new URLSearchParams({
    filter: JSON.stringify({ state: 'Held', department: 'customer-success' })
  });
  return `/customer-success/held-orders?${params.toString()}`;
});
...
const assignToMe = (order: any) => {
  console.log('Assign to me:', order);
