    ...
    <div class="px-6 erp_dashboard_wrapper">
        ...
        <div class="flex flex-wrap items-center gap-2 mb-8 md:justify-between create_grn_header_wrapper">
            ...
            <div class="flex gap-3">
                ...
                <button @click="showConfirmApproveModal = true"
                    class="flex items-center px-3 py-2 text-white gap-x-1 create_btn">
                    <span class="text-xs">Approve</span>
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M6.73537 12.322C6.55482 12.13 6.30577 12.0171 6.04238 12.0077C5.77898 11.9983 5.52253 12.0933 5.32881 12.272C5.13508 12.4507 5.01973 12.6987 5.00785 12.962C4.99597 13.2253 5.08852 13.4826 5.26537 13.678L8.87737 17.597C9.41437 18.123 10.2144 18.123 10.7114 17.627L11.0754 17.268C12.3899 15.9753 13.7029 14.6809 15.0144 13.385L15.0544 13.345C16.2801 12.1371 17.4994 10.9228 18.7124 9.70205C18.8937 9.51225 18.9931 9.25878 18.9891 8.99634C18.9852 8.73391 18.8782 8.48354 18.6913 8.29926C18.5044 8.11499 18.2526 8.01158 17.9901 8.01135C17.7276 8.01113 17.4756 8.1141 17.2884 8.29805C16.081 9.51239 14.8677 10.7207 13.6484 11.923L13.6084 11.963C12.3516 13.2053 11.0933 14.446 9.83337 15.685L6.73537 12.322Z"
                            fill="white" />
                    </svg>
                            ...
                            <tr v-for="item, index in purchaseOrder.items" :key="item.id" class="hover:bg-gray-50">
                                <td class="tracking-wider text-left w-[10%] cursor-help">
                                    <Tooltip :text="item.product_name" class="">
                                        <button class="px-2 py-3 tracking-wider text-left cursor-help">
        ...
        <SideBarModal :title="modalTitle" :is-open="isModalOpen" width="lg" :initial-form-data="formData"
            ...
            <template #default="{ formData, updateFormData, isEditItem }">
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <v-select v-model="formData.product" :options="products" label="name"
                    ...
                    <div>
                        ...
                        <v-select taggable multiple :options="warehouse_shelves" v-model="formData.warehouse_shelf"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input :value="formData.batch_no || ''"
                                @input="updateFormData({ batch_no: $event.target.value })" type="text"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input :value="formData.expiry_date || ''" @input="
                                    updateFormData({ expiry_date: $event.target.value })
                                    " type="date"
                                    class="w-full focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input :value="formData.quantity || 1" @input="
                                ...
                                " type="number"
                                :min="formData.product?.min_reorder ? formData.product?.min_reorder : 1"
                                class="w-full focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                            <p class="mt-1 text-xs text-gray-500">
                                Minimum order quantity is {{ formData.product?.min_reorder ?
                                    formData.product?.min_reorder
                                    : 1 }}
                            </p>
                        </div>
                        <div>
                            ...
                            <div class="relative">
                                <select :value="formData.inbound_type || ''"
                                    @change="updateFormData({ inbound_type: $event.target.value })"
                        ...
                        <div v-if="authStore.hasBarcodeScanning">
                            ...
                            <div class="relative">
                                <input :value="formData.barcode || ''"
                                    @input="updateFormData({ barcode: $event.target.value })" type="text"
                                    placeholder="Scan barcode" :readonly="!scanning" @keypress="handlePress"
                                    id="product_barcode"
            ...
            <template #footer="{ closeModal, formData }">
                <div class="flex justify-end space-x-3">
                    ...
                    <button @click="saveItem(formData, closeModal)" type="button" class="approve">
        ...
        <UniversalCenteredModal :show="showConfirmApproveModal" @close="showConfirmApproveModal = false">
            <template #header>
                <span class="flex items-center gap-2">
                    <h3 class="font-medium">Approve GRN</h3>
                </span>
            </template>

            <template #body>
                <p class="discard_merge_alert_body_text">
                    You are about to approve this GRN <strong>“{{ purchaseOrder?.order_no }}”</strong>. This
                    action will update it from the system.
                </p>
            </template>
            ...
            <template #footer>
                <div class="flex justify-end space-x-3 discard_merge_alert_footer">
                    <button @click="showConfirmApproveModal = false" type="button" class="cancel_btn ">
                        Cancel
                    </button>
                    <button @click="approveInvoice" type="button" class="px-4 py-2 text-sm approve_btn"
                        style="color: white">
                        Approve GRN
                    </button>
                </div>
...
const updateInvoice = () => {
    ...
    invoice.value.items.forEach(item => {
        ...
        if (!item.batch_no || item.batch_no === '' || !item.expiry_date || item.expiry_date === '' ||
            item.warehouse_shelf.length < 1) {
            toast.error("Batch information/Warehouse location is required!");
            loading.value = false;
            throw new Error("Batch information/Warehouse location is required!");
        }
    });
    let total = 0;
    invoice.value.items.forEach(item => {
...
const onProductSelect = (selected) => {
    formData.value.product_id = selected.id;
    formData.value.product_name = selected.name;
    formData.value.product = selected;
};
...
const onWarehouseSelect = (data) => {
    formData.value.warehouse_shelf = data
}
...
const approveInvoice = () => {
    ...
    invoice.value.items.forEach(item => {
        if (purchaseOrder.value.type == 'trade') {
            ...
            total += (item.quantity * item.unit_cost) - ((item.discount / 100) *
                (item.purchased_quantity * item.unit_cost))
        } else {
            total += (item.quantity * item.purchase_price) - ((item.discount / 100) *
                (item.quantity * item.purchase_price))
        }
    });
    invoice.value.total = total;
    axios.post(import.meta.env.VITE_API_URL + 'invoices/' + invoice.value.id + '/approve?current_branch=' + authStore.selectedBranch.id, invoice.value)
        .then(res => {
