<template>
  ...
  <div class="px-6 erp_dashboard_wrapper">
    ...
    <div class="flex flex-wrap items-center gap-2 mb-8 md:justify-between create_grn_header_wrapper">
      ...
      <button @click.prevent="showConfirmCreationModal = true"
        class="flex items-center px-3 text-white min-h-8 gap-x-1 create_btn">
      ...
      <div class="flex flex-wrap gap-4 md:flex-nowrap">
        <div class="w-full supplier_content">
          ...
          <v-select v-model="invoice.purchase_order_id" :options="purchaseOrders" label="order_no"
            placeholder="Search Purchase Order..." :loading="purchase_orders_loading" @update:modelValue="changeOrder"
    ...
    <div class="p-4 rounded-xl items_wrapper">
      <div class="flex justify-between gap-4 mb-4">
        ...
        <!-- <button @click="openModal" :disabled="!invoice.purchase_order_id"
          class="flex items-center p-1 rounded-md add_item_btn">
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="w-20 px-2 py-3 tracking-wider">Action</th>
              </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="item, index in purchaseOrder.items" :key="item.id" class="hover:bg-gray-50">
                ...
                <!-- <td class="px-2 py-3 tracking-wider text-left w-[10%]">
                  {{ toLocale(item.invoice_quantity_sum) }}
                </td> -->
                <td class="px-2 py-3 tracking-wider text-left w-[10%]">
                  {{ item.purchased_quantity - ((item.quantity ?? 0) + (item.invoice_quantity_sum ?? 0)) }}
                </td>
                <td class="px-2 py-3 tracking-wider text-left w-[10%]">
                  {{ item.expiry_date }}
                </td>
                ...
                <td class="px-2 py-3 tracking-wider text-left w-[10%]">
                  ₦{{ toLocale(item.total_amount ?? item.amount_paid) }}
                </td>
                <td class="px-2 py-3 tracking-wider text-left">
                  <div class="flex space-x-2">
                    <button @click="editItem(item, index)" class="text-blue-600 transition-colors hover:text-blue-800"
                      title="Edit">
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd"
                    ...
                    <button @click="deleteItem(index)" class="text-red-600 transition-colors hover:text-red-800"
                      title="Delete">
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd"
                    ...
                    <button @click="openItemSplit(index)" class="text-green-600 transition-colors hover:text-green-800"
                      title="Split Item">
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd"
    ...
    <!-- add or update item -->
    <SideBarModal :title="modalTitle" :is-open="isModalOpen" width="lg" :initial-form-data="createGrnFormData"
      ...
      <template #default="{ formData, updateFormData, isEditItem }">
        <div class="space-y-6">
          ...
          <div>
            ...
            <div class="relative">
              <v-select v-model="createGrnFormData.product" :options="products" label="name"
                placeholder="Type to search for product..." @search="onSearchProduct" :loading="product_loading"
                :clearable="true" />
            </div>
          </div>
          ...
          <div>
            ...
            <div class="relative">
              <v-select taggable multiple :options="warehouse_shelves" v-model="createGrnFormData.warehouse_shelf"
                required @input="onWarehouseSelect" />
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input v-model="createGrnFormData.batch_no" type="text" placeholder="Enter batch number"
            ...
            <div>
              ...
              <div class="relative">
                <input v-model="createGrnFormData.expiry_date" type="date"
                  class="w-full focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
              </div>
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <div class="relative">
                ...
                <input v-model="createGrnFormData.unit_cost" @input="updatePurchasePrice" type="number" step="0.01"
                  placeholder="0.00" style="padding-left: 24px;"
            ...
            <div>
              ...
              <div class="relative">
                <span class="absolute left-3 top-1.5 text-gray-500 text-lg">₦</span>
                <input v-model="createGrnFormData.purchase_price" type="number" step="0.01" placeholder="0.00"
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input v-model="createGrnFormData.quantity" type="number"
                :min="createGrnFormData.product?.min_reorder ? createGrnFormData.product?.min_reorder : 1"
                class="w-full focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
              <p class="mt-1 text-xs text-gray-500">
                Minimum order quantity is {{ createGrnFormData.product?.min_reorder ?
                  createGrnFormData.product?.min_reorder
                  : 1 }}
              </p>
            </div>
            <div>
              ...
              <div class="relative">
                <select v-model="createGrnFormData.inbound_type"
                  ...
                  <option v-for="(type, index) in inbound_types" :key="index" :value="type">{{ type }}</option>
                </select>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input v-model="createGrnFormData.discount" type="number" min="0" max="100" step="0.01" placeholder="0"
                class="w-full focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
            </div>
            <div v-if="authStore.hasBarcodeScanning">
              <label class="">Barcode</label>
              <div class="relative">
                <input v-model="createGrnFormData.barcode" type="text" placeholder="Scan barcode" :readonly="!scanning"
                  @keypress="handlePress" id="product_barcode"
    ...
    <UniversalCenteredModal :show="showSplitItemModal" @close="showSplitItemModal = false">
      ...
      <template #footer>
        <div class="flex justify-end space-x-3 discard_merge_alert_footer">
          ...
          <button @click="splitItem" type="button" class="px-4 py-2 text-sm create_btn" style="color: white">
            split
          </button>
        </div>
      </template>
    ...
    <UniversalCenteredModal :show="showConfirmCreationModal" @close="showConfirmCreationModal = false">
      <template #header>
        <span class="flex items-center gap-2">
          <h3 class="font-medium">Create GRN</h3>
        </span>
      </template>

      <template #body>
        <p class="discard_merge_alert_body_text">
          You are about to Create this GRN <strong>“{{ purchaseOrder?.order_no }}”</strong>. This
      ...
      <template #footer>
        <div class="flex justify-end space-x-3 discard_merge_alert_footer">
          ...
          <button @click="createInvoice" type="button" class="px-4 py-2 text-sm create_btn" style="color: white">
            Create
          </button>
        </div>
      </template>
...
const itemToEdit = ref<Partial<Item>>({});

const itemToSplit = ref<Partial<Item>>({
  product_id: 0,
  product_name: '',
  purchase_price: 0,
  quantity: 0,
...
const fetchInboundType = () => {
  api.get('invoices/inbound_types').then(res => {
...
const addItem = (newItem: Item, index = -1) => {
...
const showEdit = (index: number) => {
  const itm = purchaseOrder.value.items[index];
  itemToEdit.value = itm;

  if (!itm.warehouse_shelf) {
    itm.warehouse_shelf = [];
  }
};
...
const changeOrder = (value) => {
  api.get(`/purchase_orders/${value}?current_branch=${authStore.selectedBranch.id}`).then(res => {
...
const fetchPurchaseOrders = async () => {
  ...
  try {
    ...
    const res = await api.get(
      `/purchase_orders?queries[status]=0&current_branch=${authStore.selectedBranch.id}`
    );
    purchaseOrders.value = res.data.purchaseOrders;
    if (Number(route.params.id) > 0) {
      invoice.value.purchase_order_id = Number(route.params.id);
      for (let index = 0; index < purchaseOrders.value.length; index++) {
        if (purchaseOrders[index].id === route.params.id) {
          purchaseOrder.value = purchaseOrders[index];
          break;
        }
      }
      changeOrder(route.params.id);
      // invoice.value.purchase_order_id = Number(route.params.id);
      // changeOrder(route.params.id);
    }
...
const fetchProducts = async (query: string) => {
  ...
  try {
    const res = await api.get(
      `/search_products_typeahead?q=${query}&fields=id,name,price_per_pack,unit_cost,min_reorder&current_branch=${authStore.selectedBranch.id}`
    );
...
const createInvoice = () => {
  ...
  axios.post(import.meta.env.VITE_API_URL + 'invoices?current_branch=' + authStore.selectedBranch.id, invoice.value)
    .then(() => {
      ...
      router.push({ name: 'inbound.purchase_orders' })
    }).catch(err => {
      error(err);
    }).finally(() => {
      loading.value = false;
...
const onWarehouseSelect = (data) => {
  createGrnFormData.value.warehouse_shelf = data
}
...
const updatePurchasePrice = () => {
  if (createGrnFormData.value.unit_cost == 0) return;
  createGrnFormData.value.price_per_pack = Math.round(parseInt(createGrnFormData.value.unit_cost) + parseInt(7 / 100 * (createGrnFormData.value.unit_cost)));
  createGrnFormData.value.purchase_price = Math.round(parseInt(createGrnFormData.value.unit_cost) + parseInt(7 / 100 * (createGrnFormData.value.unit_cost)));
}
