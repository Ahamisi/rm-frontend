<template>
  ...
  <div>
    <Datatable :url="price_confirmation_url" :filterByDate="true" :searchable="true" :columns="priceConfirmationColumns"
      pageName="priceConfirmation" :filterField="filterField">
      <template #column="col">
        ...
        <span v-else-if="col.props.column.field === 'actions'">
          <TableActionDropdown :rowData="col.props.row" :key="col.props.row.id">
            <template #default="{ selectedItem, closeDropdown }">
              ...
              <li v-if="col.props.row.assigned_to != null" @click="unassign(col.props.row.id); closeDropdown()"
    ...
    <!-- discard delete grn modal -->
    <UniversalCenteredModal :show="deleteInvoiceWarningModal" @close="deleteInvoiceWarningModal = false">
      <template #header>
      ...
      <template #body>
        <p class="discard_merge_alert_body_text">
          You are about to delete this GRN <strong>“{{ deleteInvoiceFormData?.doc_no }}”</strong>. This
      ...
      <template #footer>
        <div class="flex justify-end space-x-3 discard_merge_alert_footer">
          <button @click="deleteInvoiceWarningModal = false" type="button" class="cancel_btn">
            Cancel
          </button>
    ...
    <UniversalCenteredModal :show="deleteInvoiceSuccessModal" @close="discardDeleteGrn">
      <template #body>
        ...
        <p class="text-center discard_merge_alert_body_text">
          The GRN <strong>“{{ deleteInvoiceFormData?.doc_no }}”</strong> has been successfully deleted from the
          system.
        </p>
      </template>
...
const price_confirmation_url = import.meta.env.VITE_API_URL + 'invoices?type=unconfirmed_price';
const priceConfirmationColumns = ref<TableColumn[]>([
  {
    label: 'Order No.',
...
const deleteInvoiceFormData = ref({
  id: 0,
  doc_no: ''
});
const deleteInvoiceWarningModal = ref(false);
const deleteInvoiceSuccessModal = ref(false);

const showDeleteWarning = (invoice) => {
  deleteInvoiceFormData.value = invoice;
  deleteInvoiceWarningModal.value = true;
}

const discardDeleteGrn = () => {
  deleteInvoiceSuccessModal.value = false;
  deleteInvoiceFormData.value = {
    id: 0,
    doc_no: ''
  };
}
...
const deleteInvoice = () => {
  deleteInvoiceWarningModal.value = false;
  loading.value = true;
  axios.delete(import.meta.env.VITE_API_URL + "invoices/" + deleteInvoiceFormData.value.id).then(response => {
    // toast.success("GRN Deleted Successfully");
    deleteInvoiceSuccessModal.value = true;
  }).catch(err => {
    error(err)
  })
...
const unassign = (id) => {
  ...
  axios.post(import.meta.env.VITE_API_URL + `invoices/${id}/unassign?current_branch=${authStore.selectedBranch.id}`).then(res => {
    ...
    toast.error("Error unassigning GRN!");
  }).finally(() => assigning.value = false);
}
