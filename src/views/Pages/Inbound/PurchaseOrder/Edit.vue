    ...
    <div class="px-6 erp_dashboard_wrapper">
        ...
        <div class="flex flex-wrap items-center gap-2 mb-2 md:justify-between create_grn_header_wrapper">
            ...
            <div class="flex items-center gap-2">
                <h3>{{ approvePage ? 'Approve' : 'Edit' }} Purchase Order</h3>
            ...
            <button @click="updatePurchaseOrder" class="flex items-center px-3 py-2 text-white gap-x-1 create_btn">
                <span class="text-xs">{{ approvePage ? "Approve" : 'Update' }}</span>
            </button>
        </div>
        ...
        <div class="p-4 mb-4 rounded-xl supplier_information">
            ...
            <div class="grid items-baseline gap-4 lg:grid-cols-4 md:grid-cols-2 sm:grid-cols-1">
                <div class="w-full supplier_content lg:col-span-4 md:col-span-2">
                ...
                <div class="w-full supplier_content">
                    ...
                    <input type="date" class="w-full form-input" v-model="purchaseOrder.purchase_date" />
                </div>
                <div class="w-full supplier_content">
                    <label>Delivery date</label>
                    <input type="date" class="w-full form-input" v-model="purchaseOrder.delivery_date" />
                </div>
                <div class="w-full supplier_content">
                    <label>Discount (%)</label>
                    <input type="number" class="w-full form-input" v-model="purchaseOrder.discount" />
                </div>
                <div class="w-full supplier_content">
                    <label>Type</label>
                    <v-select :options="types" label="name" v-model="purchaseOrder.type"
                        :reduce="type => type.id"></v-select>
                </div>
            </div>
        </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="w-[10%] px-2 py-3 tracking-wider text-left">Action</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="item, index in purchaseOrder.items" :key="item.id" class="hover:bg-gray-50">
                                ...
                                <td class="px-2 py-3 tracking-wider text-left w-[10%]">
                                    {{ ((item.quantity - item.returned) * item.unit_cost) - ((item.discount / 100) *
        ...
        <SideBarModal :title="modalTitle" :is-open="isModalOpen" width="lg" :initial-form-data="formData"
            @update:is-open="isModalOpen = $event" @close="handleClose" @form-data-change="handleFormDataChange">
            <!-- Main content -->
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <!-- <v-select
                                    ...
                                    @search="onSearcProduct"
                                    :loading="product_loading" :clearable="true"

                                    @update:modelValue="onProductSelected"
                                /> -->
                            <v-select v-model="formData.selectedProduct" :options="products" label="name"
                                placeholder="Type to search for product..." :loading="product_loading" :clearable="true"
                                @search="onSearcProduct" @update:modelValue="onProductSelected" />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input :value="formData.unit_cost || ''" @input="
                                    updateFormData({
                                        unit_cost: parseFloat($event.target.value) || null,
                                    })
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <span class="absolute left-3 top-1.5 text-gray-500 text-lg">₦</span>
                                <input :value="formData.purchase_price || ''" @input="
                                    updateFormData({
                                        purchase_price: parseFloat($event.target.value) || null,
                                    })
                                    " type="number" step="0.01" placeholder="0.00"
                                    class="w-full py-2 pl-8 pr-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent price" />
                            </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input :value="formData.quantity || 1" @input="
                                updateFormData({
                        ...
                        <div>
                            ...
                            <input :value="formData.discount || ''" @input="
                                updateFormData({
...
// This file is intentionally left empty as it serves as a placeholder for the Edit.vue component.
// The actual content and logic will be implemented in the future.
// Purchase Order / Approved PO / Create GRN
...
const getTitle = computed(() => `${title.value} / ${purchaseOrder.value?.order_no} - ${supplier_name.value}`);

const authStore = useAuthStore();
const route = useRoute();
const router = useRouter();
const toast = useToast();
const title = ref('Purchase Order / Approved PO');
...
const modifyItem = ref(false);
const modifyIndex = ref<number | boolean>(false);
const product_loading = ref(false);
const modalTitle = ref("Edit Purchase Order Item");
...
const handleFormDataChange = (data: any) => {
    ...
    // const formattedData = {
    //   product_id: this.itemToSplit.product_id,
    // 					product_name: this.itemToSplit.product_name,
    // 					purchase_price: this.itemToSplit.purchase_price,
    // 					quantity: this.itemToSplit.quantity,
    // 					purchased_quantity: this.itemToSplit.purchased_quantity,
    // 					discount: this.itemToSplit.discount,
    // 					batch_no: "",
    // 					expiry_date: "",
    // 					warehouse_shelf: [],
    // 					unit_cost: this.itemToSplit.unit_cost,
    // 					barcode: '',
    // }
    // addItem();
    // formData.value = data;
...
const saveItem = (data: any, closeModal: () => void) => {
    if (parseFloat(data.purchase_price) < parseFloat(data.unit_cost)) {
        alert('Product price cannot be lesser than product unit_cost!');
        return;
    }

    let modifiedItems = purchaseOrder.value.items.map((product, index) => {
        if (modifyIndex.value == index) {
            return {
                ...product,
                ...data,
            }
    ...
    modifyItem.value = false;
    modifyIndex.value = false;
    closeModal();
};
...
const editItem = (item, index) => {
    ...
    modifyIndex.value = false;
    products.value = [item.product, ...products.value.filter(p => p.id !== item.id)];
    formData.value = { ...item, purchase_price: item.product.price_per_pack };
    setTimeout(() => {
...
const onSearcProduct = (query: string) => {
...
const onProductSelected = (product) => {
    if (product) {
        ...
        products.value = [product, ...products.value.filter(p => p.id !== product.id)];
...
const updatePurchaseOrder = () => {
    let msg = 'Are you want to ' + (approvePage.value ? 'approve' : 'update') + ' this Purchase Order?';
    if (!confirm(msg)) return;

    disableForm.value = true;
    if (approvePage.value) purchaseOrder.value.status = 1;
    axios.put(import.meta.env.VITE_API_URL + `purchase_orders/${purchaseOrder.value.id}?current_branch=${authStore.selectedBranch.id}`, purchaseOrder.value)
        .then(res => {
            purchaseOrder.value = {
                id: route.params.id,
                supplier_id: '',
            ...
            const res_msg = approvePage.value ? "Purchase order approved successfully" : "Purchase order updated successfully";
