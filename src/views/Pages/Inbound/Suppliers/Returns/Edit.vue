<template>
    ...
    <div class="px-6 erp_dashboard_wrapper">
                            ...
                            <tr v-for="item, index in invoice.items" :key="'item' + index" class="hover:bg-gray-50">
                                <td class="px-2 py-3 tracking-wider text-left w-[10%] cursor-help">
                                    <input type="checkbox"
                                        ...
                                        :checked="getItemIndex(item.id) !== -1" @change="checkItem($event, index)">
                                </td>
        ...
        <UniversalCenteredModal :show="showConfirmUpdateModal" @close="showConfirmUpdateModal = false">
            <template #header>
                <span class="flex items-center gap-2">
                    <h3 class="font-medium">Update Supplier's Return</h3>
                </span>
            </template>

            <template #body>
                <p class="discard_merge_alert_body_text">
                    You are about to update this Supplier's Return <strong>“{{ invoice?.doc_no }}”</strong>. This
                    action will update it on the system.
                </p>
            </template>

            <template #footer>
                <div class="flex justify-end space-x-3 discard_merge_alert_footer">
                    <button @click="showConfirmUpdateModal = false" type="button" class="cancel_btn">
                        Cancel
                    </button>
                    <button @click="updateReturn" type="button" class="px-4 py-2 text-sm create_btn"
                        style="color: white">
                        Update
                    </button>
                </div>
