      ...
      <div class="intro_search_wrapper">
        ...
        <div class="relative grid items-baseline grid-cols-1 gap-2 md:grid-cols-7 search_content">
          ...
          <div class="relative p-0 m-0 md:col-span-4">
            <svg width="21" height="20" viewBox="0 0 21 20" fill="none" xmlns="http://www.w3.org/2000/svg" class="absolute top-2 left-1">
              <path fill-rule="evenodd" clip-rule="evenodd"
            ...
            <input type="text" v-model="search" class="w-full pr-2 pl-7 py-1.5 border border-gray-300 rounded-md focus:outline-none">
          </div>
          <button class="text-white" @click="performSearch">Search</button>
        </div>
      </div>
    </div>
        ...
        <div v-if="activeTab === 'details'" class="space-y-6">
          ...
          <div class="mt-8">
            ...
            <div class="flex items-center justify-between px-6 mt-4 order_total">
              ...
              <span class="">{{ selectedOrder.total_amount }}</span>
            </div>
          </div>
        </div>
