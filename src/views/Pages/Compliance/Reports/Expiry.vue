<template>
    <div class="erp_dashboard_wrapper">
        <div class="grey_bg">
            <!-- Header -->
            <PageTitle title="Report / Expiry Report / Expired" class="px-6" />

            <!-- tabs -->
            <Tabs :tabs="expiryTabs" @tab-changed="handleTabChange" :defaultTab="activeTab">
                <!-- Download Button in tabs line -->
                <div class="flex items-center gap-3 ml-auto mb-1">
                    <Button 
                        type="blue-btn"
                        :onClick="downloadExpiryReport"
                        classStyle="px-4 py-2"
                    >
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M10.687 17.292C10.5956 17.1997 10.4868 17.1264 10.3669 17.0764C10.247 17.0264 10.1184 17.0007 9.9885 17.0007C9.8586 17.0007 9.72998 17.0264 9.61009 17.0764C9.49019 17.1264 9.3814 17.1997 9.29 17.292C9.10466 17.4792 9.0007 17.732 9.0007 17.9955C9.0007 18.259 9.10466 18.5118 9.29 18.699L11.254 20.679C11.3546 20.7807 11.4744 20.8613 11.6064 20.9164C11.7384 20.9715 11.88 20.9998 12.023 20.9998C12.166 20.9998 12.3076 20.9715 12.4396 20.9164C12.5716 20.8613 12.6914 20.7807 12.792 20.679L14.711 18.746C14.8966 18.5587 15.0008 18.3057 15.0008 18.042C15.0008 17.7783 14.8966 17.5253 14.711 17.338C14.6196 17.2455 14.5107 17.1721 14.3907 17.122C14.2708 17.0719 14.142 17.0462 14.012 17.0462C13.882 17.0462 13.7532 17.0719 13.6333 17.122C13.5133 17.1721 13.4044 17.2455 13.313 17.338L12.023 18.638L10.687 17.292Z" fill="white"/>
                            <path d="M13.001 19.993L13 10.006C13 9.451 12.552 9 12 9C11.448 9 11 9.45 11 10.007L11.001 19.994C11.001 20.549 11.449 21 12.001 21C12.553 21 13.001 20.55 13.001 19.993Z" fill="white"/>
                            <path fill-rule="evenodd" clip-rule="evenodd" d="M7.938 5.48C7.68111 5.4383 7.42125 5.41757 7.161 5.418C4.356 5.418 2 7.62 2 10.498C2 13.409 4.385 16 7.1 16H9.981V14.007H7.1C5.443 14.007 3.985 12.344 3.985 10.499C3.985 8.721 5.454 7.412 7.089 7.412H7.101C7.49 7.412 7.787 7.462 8.071 7.562L8.241 7.625C8.846 7.873 9.116 7.379 9.116 7.379L9.266 7.112C9.996 5.765 11.467 5.016 12.982 4.992C13.9871 5.00203 14.9543 5.37742 15.703 6.04812C16.4517 6.71882 16.9309 7.63901 17.051 8.637L17.097 8.977C17.097 8.977 17.168 9.502 17.762 9.502C17.775 9.502 17.774 9.507 17.785 9.507H18.039C19.175 9.507 20.015 10.466 20.015 11.665C20.015 12.872 19.028 14.007 17.945 14.007H13.981V16H17.945C20.105 16 22 13.955 22 11.665C22 9.665 20.688 8.002 18.862 7.591C18.155 4.884 15.809 3.039 12.976 3C11.001 3.02 9.075 3.9 7.938 5.48Z" fill="white"/>
                        </svg>
                        Download Expiry Report
                    </Button>
                </div>
            </Tabs>
        </div>
        
        <!-- contents -->
        <div class="px-6 mt-0 bg-white tab_contents min-h-[calc(100vh-190px)]">
            <Datatable 
                :items="expiryData" 
                :filterByDate="false" 
                :filterFields="filterFields" 
                :columns="columns" 
                pageName="Expiry Report"
                :key="`expiry-${activeTab}-${childKey}`"
            />
        </div>
    </div>
</template>

<script setup lang="ts">
import { ref, nextTick } from 'vue'
import Datatable from '@/views/Components/Datatable/Datatable.vue'
import PageTitle from '@/views/Components/header/PageTitle.vue'
import Tabs from '@/views/Components/Tabs.vue'
import Button from '@/views/Components/ui/Button.vue'
import type { TableColumn, FilterFields, FilterField } from '@/types'

// Types
interface ExpiryData {
    id: number
    productName: string
    batchNo: string
    expiryDate: string
    quantity: number
}

// Tab state
const activeTab = ref('Expired')
const childKey = ref(0)

// Tab configuration
const expiryTabs = ref([
    { name: 'Expired', count: 14 },
    { name: '3 Months', count: 8 },
    { name: '6 Months', count: 12 },
    { name: '1 Year', count: 6 },
    { name: '1 Year and 6 Months', count: 4 }
])

// Mock expiry data
const expiryData = ref<ExpiryData[]>([
    { id: 87, productName: "Sivocid", batchNo: "211115", expiryDate: "16/01/2025", quantity: 4 },
    { id: 54, productName: "Augmentin 228 syrup", batchNo: "211115", expiryDate: "16/01/2025", quantity: 4 },
    { id: 25, productName: "Bunto", batchNo: "211115", expiryDate: "16/01/2025", quantity: 4 },
    { id: 30, productName: "Sivocid", batchNo: "211115", expiryDate: "16/01/2025", quantity: 4 },
    { id: 51, productName: "Augmentin 228 syrup", batchNo: "211115", expiryDate: "16/01/2025", quantity: 4 },
    { id: 21, productName: "Bunto", batchNo: "211115", expiryDate: "16/01/2025", quantity: 4 },
    { id: 35, productName: "Sivocid", batchNo: "211115", expiryDate: "16/01/2025", quantity: 4 },
    { id: 23, productName: "Augmentin 228 syrup", batchNo: "211115", expiryDate: "16/01/2025", quantity: 4 },
    { id: 33, productName: "Bunto", batchNo: "211115", expiryDate: "16/01/2025", quantity: 4 },
    { id: 88, productName: "Sivocid", batchNo: "211115", expiryDate: "16/01/2025", quantity: 4 },
    { id: 81, productName: "Augmentin 228 syrup", batchNo: "211115", expiryDate: "16/01/2025", quantity: 4 },
    { id: 80, productName: "Bunto", batchNo: "211115", expiryDate: "16/01/2025", quantity: 4 },
    { id: 5, productName: "Sivocid", batchNo: "211115", expiryDate: "16/01/2025", quantity: 4 },
    { id: 55, productName: "Augmentin 228 syrup", batchNo: "211115", expiryDate: "16/01/2025", quantity: 4 }
])

// Table columns
const columns = ref<TableColumn[]>([
    {
        label: 'ID',
        field: 'id',
        sortable: true
    },
    {
        label: 'Product Name',
        field: 'productName',
        sortable: true
    },
    {
        label: 'Batch No.',
        field: 'batchNo',
        sortable: true
    },
    {
        label: 'Expiry Date',
        field: 'expiryDate',
        sortable: true
    },
    {
        label: 'Quantity',
        field: 'quantity',
        sortable: true
    }
])

// Filter fields
const filterFields = ref<FilterFields<FilterField>>({
    productName: {
        type: 'text',
        label: 'Product Name',
        placeholder: 'Search by product name'
    },
    batchNo: {
        type: 'text',
        label: 'Batch No.',
        placeholder: 'Search by batch number'
    }
})

// Tab change handler
const handleTabChange = (tab: string | { name: string; count: number }, index: number) => {
    activeTab.value = typeof tab === 'string' ? tab : tab.name;
    
    // Force reactivity update for tables
    nextTick(() => {
        childKey.value += 1;
    });
};

// Download function
const downloadExpiryReport = () => {
    console.log('Downloading expiry report...');
    // TODO: Implement actual download functionality
};
</script>

<style scoped>
.grey_bg {
    background: rgba(247, 248, 249, 1);
}

.tab_contents {
    height: calc(100vh - 190px) !important;
    overflow: hidden !important;
}
</style>
