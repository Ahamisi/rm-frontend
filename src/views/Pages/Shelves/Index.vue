<template>
    ...
    <div class="erp_dashboard_wrapper">
        <!-- Header -->
        <PageTitle title="Location / Shelves" class="px-6" />

        <div class=" min-h-[calc(100vh-190px)] px-6 bg-white">
            <Datatable :url="url" :filterByDate="false" :columns="newColumns" pageName="Shelve" :key="childKey">
                <template v-slot:header_actions>
                    <button @click.prevent="isCreateShelveModalOpen = true"
                        ...
                        <span>Create Shelve</span>
                    </button>
                </template>
                <template #column="col">
                    <span v-if="col.props.column.field == 'actions'">
                        <div class="flex items-center justify-center gap-3">
                            <button type="button" @click.prevent="editShelve(col.props.row)">
                            ...
                            <button type="button" @click.prevent="openConfirmDeleteShelve(col.props.row)">
                                <svg width="25" height="24" viewBox="0 0 25 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" clip-rule="evenodd"
        ...
        <!-- discard delete shelve modal -->
        <UniversalCenteredModal :show="showDiscardDeleteShelveModal" @close="showDiscardDeleteShelveModal = false">
            <template #header>
            ...
            <template #body>
                <p class="discard_merge_alert_body_text">
                    You are about to delete this shelf <strong>“{{ deleteShelveFormData.name }}”</strong>. This
            ...
            <template #footer>
                <div class="flex justify-end space-x-3 discard_merge_alert_footer">
                    <button @click="showDiscardDeleteShelveModal = false" type="button" class="cancel_btn">
                        Cancel
                    </button>
                    <button @click="deleteShelve" type="button" class="text-white delete_btn">
                        Delete Shelf
                    </button>
                </div>
            </template>
        ...
        <!-- deleted shelve success message modal -->
        <UniversalCenteredModal :show="showDeletedShelveSuccessModal" @close="closeDeleteShelveSuccessModal">
            <template #body>
                ...
                <h3 class="text-center">Shelve Deleted</h3>
                <p class="text-center discard_merge_alert_body_text">
                    The shelve <strong>“{{ deleteShelveFormData.name }}”</strong> has been successfully deleted from the
                    system.
                </p>
            </template>
            ...
            <template #footer>
                <div class="flex justify-end space-x-3 discard_merge_alert_footer">
                    ...
                    <button @click="closeDeleteShelveSuccessModal" type="button" class="text-white approve_btn">
                        Done
                    </button>
                </div>
            </template>
        ...
        <Create :isOpen="isCreateShelveModalOpen" @created="childKey++; isCreateShelveModalOpen = false"
            @close="isCreateShelveModalOpen = false" @discarded="isCreateShelveModalOpen = false" />
        <Edit :isOpen="isEditShelveModalOpen" :shelve="editShelveFormData"
            @updated="childKey++; isEditShelveModalOpen = false" @close="isEditShelveModalOpen = false"
            @discarded="isEditShelveModalOpen = false" />
    </div>
</template>
...
const isCreateShelveModalOpen = ref(false);
const showDiscardDeleteShelveModal = ref(false);
const showDeletedShelveSuccessModal = ref(false);
...
const url = computed(() => import.meta.env.VITE_API_URL + 'shelves');
...
const deleteShelveFormData = ref({
    id: 0,
    name: ''
})
...
const deleteShelve = (id) => {
    showDiscardDeleteShelveModal.value = false;
    if (!deleteShelveFormData.value?.id) {
        toast.error("Please select a shelve to delete");
        return;
    }
    loading.value = true;
    axios.delete(import.meta.env.VITE_API_URL + `shelves/${deleteShelveFormData.value.id}?current_branch=${authStore.selectedBranch.id}`).then(response => {
        showDeletedShelveSuccessModal.value = true;
    }).catch(err => {
        let errorMessage = '';
...
const closeDeleteShelveSuccessModal = () => {
    deleteShelveFormData.value = {
        id: 0,
        name: ''
    }
    showDeletedShelveSuccessModal.value = false;
    loading.value = false;
    childKey.value++;
}

const openConfirmDeleteShelve = (shelve) => {
    deleteShelveFormData.value = shelve;
    showDiscardDeleteShelveModal.value = true;
}


const editShelve = (shelve) => {
    editShelveFormData.value = shelve;
    isEditShelveModalOpen.value = true;
}
</script>
