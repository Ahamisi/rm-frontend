<template>
    ...
    <!-- create shelve modal -->
    <SideBarModal title="Create Shelf" :is-open="isCreateShelveModalOpen" width="lg"
        @update:is-open="isCreateShelveModalOpen = $event" ref="createShelfModal"
        :initial-form-data="createShelveFormData" @close="closeModal">
        ...
        <template #default="{ formData, updateFormData, isEditItem }">
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" v-model="createShelveFormData.name" placeholder="Enter a shelf name"
                ...
                <div v-if="authStore.hasBarcodeScanning">
                    ...
                    <div class="relative">
                        <input v-model="createShelveFormData.barcode" type="text" placeholder="Scan barcode"
        ...
        <template #footer="{ closeModal, formData, showDiscardWarning, triggerDiscardWarning }">
            <div class="flex justify-end space-x-3">
                ...
                <button @click="createShelve" type="button" class="approve_btn">
                    Create
                </button>
            </div>
        </template>
    ...
    <UniversalCenteredModal :show="showDiscardCreateShelveModal" @close="showDiscardCreateShelveModal = false">
        <template #header>
        ...
        <template #body>
            <p class="discard_merge_alert_body_text">
                You are about to leave the <strong>"Create Shelve"</strong> process. Any unsaved information will be
                lost.
            </p>
        </template>
        ...
        <template #footer>
            <div class="flex justify-end space-x-3 discard_merge_alert_footer">
                <button @click="showDiscardCreateShelveModal = false" type="button" class="cancel_btn">
                    Cancel
                </button>
    ...
    <UniversalCenteredModal :show="showCreateShelveSuccessModal" @close="closeShelveCreatedSuccessModal">
        <template #body>
            ...
            <h3 class="text-center">Shelve Created</h3>
            <p class="text-center discard_merge_alert_body_text">
                The shelve <strong>“{{ createShelveFormData.name }}”</strong> has been successfully added to the
                system.
            </p>
        </template>
        ...
        <template #footer>
            <div class="flex justify-end space-x-3 discard_merge_alert_footer">
                ...
                <button @click="closeShelveCreatedSuccessModal" type="button" class="text-white approve_btn">
                    Done
                </button>
            </div>
        </template>
...
watch(() => props.isOpen, (newVal, oldVal) => {
    isCreateShelveModalOpen.value = newVal
});

const createShelfModal = ref(null)
const isCreateShelveModalOpen = ref(false);
const showCreateShelveSuccessModal = ref(false);
const showDiscardCreateShelveModal = ref(false);
// const showCreateShelveSuccessModal = ref(false);
const authStore = useAuthStore();
const toast = useToast();
const createShelveFormData = ref({
    name: '',
    barcode: ''
})
...
const { hasChanged, reset } = useDataChangeTracker(createShelveFormData);

const handleShowDiscardShelfWarningModal = () => {
    if (hasChanged.value) {
        showDiscardCreateShelveModal.value = true;
    } else {
        discardChanges()
    }
}
...
const closeModal = () => {
    if (isCreateShelveModalOpen.value) createShelfModal.value?.closeModal();
    emit('close');
}
...
const closeShelveCreatedSuccessModal = async () => {
    createShelveFormData.value = {
        name: "",
        barcode: ""
    };
    await nextTick();
    reset()
    showDiscardCreateShelveModal.value = false;
...
const createShelve = () => {
    loading.value = true;
    axios.post(import.meta.env.VITE_API_URL + 'shelves?current_branch=' + authStore.selectedBranch?.id, createShelveFormData.value)
        .then(res => {
